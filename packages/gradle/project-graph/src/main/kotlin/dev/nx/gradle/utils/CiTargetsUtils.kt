...
private object KotlinPsiManager {
  ...
  private val environment: KotlinCoreEnvironment? = try {
    KotlinCoreEnvironment.createForProduction(
        disposable, sharedCompilerConfiguration, EnvironmentConfigFiles.JVM_CONFIG_FILES)
  } catch (e: Exception) {
    null
  }
  val psiManager: PsiManager? = environment?.let { PsiManager.getInstance(it.project) }
}
    ...
    val ktFile =
        try {
          synchronized(KotlinPsiManager) {
            val psiManager = KotlinPsiManager.psiManager
            if (psiManager == null) {
              logger?.warn("KotlinPsiManager not available for ${file.name} - falling back to regex parsing")
              return null
            }
